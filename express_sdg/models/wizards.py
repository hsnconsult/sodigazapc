...
class ExpressdgMoyenpaiement(models.TransientModel):
    ...
    def get_pointvente(self):
        pointvente = self.env['pos.config'].search([])[0]
        return pointvente.id
    #fields
    ...
    point_vente=fields.Many2one('pos.config', required=False, string="Point de vente", default=get_pointvente)
...
class ExpressdgStocks(models.TransientModel):
    ...
    def get_entrepot(self):
        entrepot = self.env['stock.warehouse'].search([])[0]
        return entrepot.id
    #fields
    date_debut = fields.Date(string="Début", required=True, default=fields.Date.today)
    date_fin = fields.Date(string="Fin", required=True, default=fields.Date.today)
    entrepot = fields.Many2one('stock.warehouse', required=False, string="Entrepôt", default = get_entrepot)
...
class ExpressdgMoyenpaiementabstract(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data.get("pointdevente"):
            condition+=" and pp.session_id = ps.id and ps.config_id = poc.id and poc.id="+str(data.get("pointdevente"))+" "
        query = """ SELECT ppm.name as moyenpaiement,sum(pp.amount) as montant
            FROM public.pos_payment as pp,
                ...
                public.pos_session ps,
                public.pos_config poc
            WHERE pp.payment_method_id=ppm.id
            """+condition+""" 
            and pp.payment_date::date between %s and %s
            group by ppm.name """
...
class ExpressdgVentesposabstract(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data.get("caissiere"):
            condition+=" and ru.id="+str(data.get("caissiere"))
        if data.get("rayon"):
            condition+=" and esr.id="+str(data.get("rayon")) 
        if data.get("famille"):
            condition+=" and esf.id="+str(data.get("famille"))
        if data.get("categorie"):
            condition+=" and pc.sid="+str(data.get("categorie"))
        if data.get("produit"):
            condition+=" and pol.product_id="+str(data.get("produit"))
        if data.get("pointdevente"):
            condition+=" and poc.id="+str(data.get("pointdevente"))
        queryq="""SELECT pt.default_code,pt.name as produit,
        esr.name as rayon,esf.name as famille,pc.name as categorie,
        sum(pol.qty) as qtv,sum(pol.price_subtotal)::decimal(16,2) as tht,
        (sum(pol.price_subtotal_incl)-sum(pol.price_subtotal))::decimal(16,2) as mtaxe,        
        sum(pol.price_subtotal_incl)::decimal(16,2) as ttc,
        sum(pol.qty*ct.cout)::decimal(16,2) as caht,
        sum(pol.qty*ct.cout*pt.taxetmpwell/100)::decimal(16,2) as mtaxeca,
        sum(pol.qty*ct.cout+pol.qty*ct.cout*pt.taxetmpwell/100)::decimal(16,2) as catc,
        sum(pol.price_subtotal-pol.qty*ct.cout)::decimal(16,2) as mgt,
        sum(pol.price_subtotal-pol.qty*ct.cout)/sum(pol.qty)::decimal(16,2) as mgu
        ...
        """+condition+"""  
        group by pt.default_code,pt.name,esr.name,esf.name,pc.name
...
class ExpressdgVentesposxlsxabstract(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        col=0
        row=0
        tht = mtaxe = ttc = caht = mtaxeca = catc = mgt = mgu = 0
        ...
        for line in rec:
            row+=1
            tht = tht + line.get("tht")
            mtaxe = mtaxe + line['mtaxe']
            ttc = ttc + line.get("ttc")
            caht = caht + line['caht']
            mtaxeca = mtaxeca + line.get("mtaxeca")
            catc = catc + line['catc']
            mgt = mgt + line.get("mgt")
            mgu = mgu + line.get("mgu")
        ...
        worksheet.write(row, col+5, '',boltdsep)
        worksheet.write(row, col+6, tht,boltdsep)
        worksheet.write(row, col+7, mtaxe,boltdsep)
        worksheet.write(row, col+8, ttc,boltdsep)
        worksheet.write(row, col+9, caht,boltdsep)
        worksheet.write(row, col+10, mtaxeca,boltdsep)
        worksheet.write(row, col+11, catc,boltdsep)
        worksheet.write(row, col+12, mgt,boltdsep)
        worksheet.write(row, col+13, '',boltdsep)
...
class ExpressdgStocksabstract(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        lot.id as lot_id,lot.name as lot_name,ct.cout as price_unit,lot.expiration_date::date as dateuse
        FROM stock_move_line s, stock_location l, 
        ...
        AND s.date::date<%s 
        GROUP BY s.product_id,lot.id,lot.name,ct.cout
        ),
        stinitsortie as  (
        SELECT s.product_id, sum(s.qty_done) as qty_done,
        lot.id as lot_id,lot.name as lot_name,lot.expiration_date::date as dateuse
        FROM stock_move_line s, stock_location l,product_product p, product_template t, 
        ...
        ,public.stock_production_lot lot, coutarticle ct
        WHERE s.location_dest_id = l.id
        and lot.id=s.lot_id
        AND p.product_tmpl_id = t.id
        and p.id=s.product_id
        ...
        coalesce(t.stock_minimal,0) as stockmin, coalesce(stf.price_unit,0)*coalesce(stf.stockfinal,0) as vsca
